...
def nb_train_smoothing(X, Y, vocab_size, num_classes, alpha):
    ...
    class_word_prob.fill(alpha)
    
    
    #print ("NEW ::",class_word_prob)
    ...
    class_prob[0][0]=zero/len(Y)
    class_prob[1][0]=one/len(Y)
    
    #print ("prior::",class_prob)
    
    #print ("shape::",dict(X[0]))
    for i in range(len(X)):
      freq=dict(X[i])
      #print ("Label::",Y[i])
      if Y[i]==0:
         for k in freq.keys():
            class_word_prob[0][k] +=freq[k]
      elif Y[i]==1:
         for k in freq.keys():
            class_word_prob[1][k] +=freq[k]
    
    total_zero=sum(class_word_prob[0])
    total_one=sum(class_word_prob[1])
    
    class_word_prob[0] = [x /total_zero  for x in class_word_prob[0] ] 
    class_word_prob[1] = [x /total_one  for x in class_word_prob[1] ]         
    print ("probabilities::",class_word_prob)
...
def nb_predict(X, class_prob, class_word_prob):
    ...
    for i in range(len(X)):
      ...
      class1=np.log(class_prob[0][0])
      class2=np.log(class_prob[1][0])
      for key in diction.keys():
         #print ("key::",key)
         
               #print ("hi")
         if class_word_prob[0][key]==0 and class_word_prob[1][key]!=0:
            class_word_prob[0][key]=-1*np.exp(50)
         if class_word_prob[1][key]==0 and class_word_prob[0][key]!=0:
            class_word_prob[1][key]=-1*np.exp(50)
         if class_word_prob[0][key]!=0 and class_word_prob[1][key]!=0:
            if class_word_prob[0][key]!=-1*np.exp(50):
               class1 += diction[key]*np.log(class_word_prob[0][key])
            else:
               class1 += diction[key]*-1*np.exp(50)
            if class_word_prob[1][key]!=-1*np.exp(50):
               class2 += diction[key]*np.log(class_word_prob[1][key])
            else:
               class2 += diction[key]*-1*np.exp(50)
      if class1>class2:
         Ypred.append(0)
      else:
         Ypred.append(1)
